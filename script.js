...
$(document).ready(function () {
    ...
    $('#sortIcon').find('path').attr('d', path_d_asc);
    ...
    $('[name="sort"]').on('click', function () {
        ...
        if ($(this).hasClass("asc")) {
            ...
            $(this).addClass("desc");
            $(this).find('path').attr("d", path_d_desc);
        }
        else {
            $(this).removeClass("desc");
            $(this).addClass("asc");
            $(this).find("path").attr("d", path_d_asc);
        }
        sort($sortBy, $sortDir);
    })


    //TODO: fix sorting by rating (currently due to str comparison it orders as: 1,10,2,3,... instead of 1,2,3,...,10)
    function sort(sortBy, sortDir) {
        ...
        rows.sort(function (first, second) {
            ...
            if (sortDir == "asc") {
                return (first_val < second_val) ? -1 : (first_val > second_val) ? 1 : 0;
            }
